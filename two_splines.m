...
for i = 1:input_length
    ...
    model_output(i) = model_output1(i) + model_output2(i) ; 
...
multiplier_2_array(iter) = multiplier2 ;
end
...
for i = 1:length(x)
    ...
    model_output2 = mean(multiplier_2_array) * parameter_array_u * C * control_point_array2(index:index + 2, 1);
    y(i) = model_output1 + model_output2;
    z(i) = f(x(i));
end
plot(x,y) ; hold on ; plot(x,z,'r') ;

function physical_output = f(x)
    k1 = 3 ; k2 = 3 ; h1 = 5 ; h2 = 0.5 ; c1 = -0.8 ; c2 = 0.5 ;
